...
const searchGitHub = async (package_name: string,
    executeShellCommand: Fig.ExecuteCommandFunction,
    ...
    const generalUrl = "https://api.github.com/search/repositories?q=$NAME$+in:name"
    const versionsUrl = "https://api.github.com/repos/$FULL_NAME$/releases"

    try {
        const envs = (await executeShellCommand({command: envVarGenerator.script[0], args: envVarGenerator.script.slice(1)}))
            .stdout
            .split('\n').map(l => ({name: l.split('=')[0].trim(), value: l.split('=')[1].trim()}))
        
        if (envs['GITHUB_TOKEN']) {
            query.push('-H')
            query.push('Authorization: Bearer $TOKEN$')
            query[query.length - 1] = query[query.length - 1].replace('$TOKEN$', envs['GITHUB_TOKEN'])
        }

        const url = package_name[package_name.length - 1] === "@" ? versionsUrl : generalUrl
        query.push(url)
        query[query.length - 1] = query[query.length - 1].replace('$NAME$', package_name)
        query[query.length - 1] = query[query.length - 1].replace('$FULL_NAME$', package_name.slice(0, package_name.length - 1))
        ...
        if (package_name[package_name.length - 1] === "@") {
            ...
            return [...new Set(
                JSON.parse(stdout)
                .filter(e => e.assets.length > 0).slice(0, 200)
                .map(e => ({
            ...
            return [...new Set(JSON.parse(stdout).items.slice(0, 200)
            .map(entry => ({
...
const searchBackend = async (
  ...
  switch (backend) {
    ...
    case "ubi":
        return await searchGitHub(customContext[customContext.length - 1], executeShellCommand, shellContext)
    default:
      return [];
  }
};
...
const getBackends = async (executeShellCommand: Fig.ExecuteCommandFunction): Promise<string[]> => {
    const { stdout, stderr, status } = await executeShellCommand({
        command: 'sh', args: ['-c', 'mise backends ls']
    })
    if (status != 0) {
        return [stderr]
    }
...
const toolVersionGenerator: Fig.Generator = {
  ...
  custom: async (
    ...
    const backendSuggestions = (await getBackends(executeShellCommand))
        .map(backend => ({name: backend, description: 'Backend'}))
